...
- Email: Resend adapter gated on `RESEND_API_KEY`; ALL sends go through
  ...
  from previews
- Sessions: `payload-token` cookie, 1 year for `user` / 2 h for admin
  roles, PLUS a rolling refresh: `POST /api/auth/refresh` re-signs the
  cookie for the full duration once the token is 15+ min old (pure rules
  in `src/lib/auth/sessionRefresh.ts`, called throttled from
  `PwaProvider`), so an installed app that gets opened now and then never
  logs out. The 1-year figure is promised in the privacy policy's cookie
  table — change both together. Set `SESSION_COOKIE_DOMAIN=.zicha.travel`
  so one session works across chata subdomains (also required for OAuth
  started on a subdomain)
...
### PWA — installable app, offline mode, update hardening

The site installs as an app (manifest + service worker + icons); design
notes live in `src/lib/pwa.ts`, pure parts unit-tested in
`tests/int/pwa.int.spec.ts` and `sessionRefresh.int.spec.ts`.

- **The installed app is always zicha.travel, never a chata subdomain**
  (a subdomain install would rot when the trip ends). The platform binds
  every install to the origin it starts from, so: the apex/preview/
  localhost manifest starts at `/`; a single-chata subdomain still gets a
  manifest (native "Add app" keeps working) but its `start_url` is
  `/app-start`, a bridge route that 307s to the apex derived from
  `SESSION_COOKIE_DOMAIN`. Launching such an install lands on
  zicha.travel with the browser's origin bar visible (out-of-scope
  navigation — the unavoidable cost); installing from the apex itself is
  clean. `/manifest.webmanifest` is a host-aware route handler (single-
  chata detected via the middleware's `x-matched-chata-slug` header);
  preview deployments get the name "zicha.travel (preview)" so the two
  installs are distinguishable. Icons in `public/icons/` are generated by
  `pnpm icons:pwa` (sharp; 192/512 + maskable + apple-touch, committed).
- **Service worker can never go stale — keep all four locks**: /sw.js is
  a route handler serving `src/lib/serviceWorkerSource.ts` with the build
  id baked in (new deploy = new bytes) and `Cache-Control: no-store`;
  registration uses `updateViaCache: 'none'`; `PwaProvider` calls
  `registration.update()` on focus/visibility + hourly; the worker
  `skipWaiting()`s and `clients.claim()`s, and deletes other builds'
  `zt-*` caches on activate. Not registered in dev or unversioned builds.
- **Offline**: allowlist-only caching — navigations and
  `/api/chatas/slug/*` network-first with cached fallback (that slug
  payload is the "important data": Finance/Informace stay readable
  offline), `/_next/static` cache-first, images/fonts/media
